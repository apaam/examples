...
# contact model
cnt_model_1 = LinearSpring(2.0e6, 1.0e6, 0.3, 0.0)
cnt_model_1.label = "cnt_model_1"
cnt_model_1_prt = sim.scene.InsertContactModel(cnt_model_1)

cnt_model_2 = LinearSpring(2.0e6, 1.0e6, 0.3, 0.0)
...
tri_mesh = TriMesh()
tri_mesh.InitFromSTL(dir_path + "/data/particle_template.stl")
tri_mesh.Decimate(200)
tri_mesh.MakeConvex()
tri_mesh.AlignAxes()
tri_mesh.SetSize(0.01)
particle_shape_ptr = sim.scene.InsertShape(tri_mesh)

# generate particles
pack_generator = PackGenerator()
particle_list = pack_generator.GetGridPack(
    0.2, 0.2, 1.0, 0, 0, 0.52, 12, 12, 60, particle_shape_ptr)
